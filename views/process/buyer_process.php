...
//    $status = $datamanipulation->updateNotice($id, $notice);
    ...
    if(!empty($fileName)){
        $status = $datamanipulation->updateSellerProfileWithImage($name,$email,$phone,$shop_name,$address,$password,$image,$id);

    }else{
        $status = $datamanipulation->updateSellerProfileWithOutImage($name,$email,$phone,$shop_name,$address,$password,$id);
    }
    ...
    if ($status) {
        $_SESSION["UdateMsg"] = "<div class=\"alert alert-warning alert-dismissible fade show\">
                          ...
                          <span>
                            <b> Updated - </b>  Your Data Successfully. </span>
                        </div>";
...
if(isset($_GET['delete_notice'])){
    ...
    if($status){
        Utility::redirect("../../views/nursery_woner/notice.php");
...
if(isset($_GET['confirm_user_by_building_woner'])){
    ...
    if($status){
        Utility::redirect("../../views/nursery_woner/pending_request.php");
...
if(isset($_GET['confirm_user_by_building_woner_from_member_list'])){
    ...
    if($status){
        Utility::redirect("../../views/nursery_woner/membership_subscription.php");
...
if(isset($_POST['coutnRating'] )){
    ...
    $data = $datamanipulation->insertRate($user_id,$client_id,$coutnRating);
    $http_reffer = $_SERVER['HTTP_REFERER'];
